...
| Healthcheck production | ✅ | `GET /api/health` — `SELECT 1` Prisma, HTTP 200 si sain / 503 si base down, `Cache-Control: no-store`, aucun secret dans le JSON. Hors rate limiting. **UptimeRobot / cron** : pointer `https://<domaine>/api/health`, type HTTPS, alerter si statut ≠ 200 ; intervalle 5 min (ne pas descendre sous 1 min). |
